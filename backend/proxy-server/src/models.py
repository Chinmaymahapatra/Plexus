...
from sqlalchemy import (
    ...
    ForeignKey, Enum, UniqueConstraint, Index, Numeric
)
...
class Developer(Base):
    ...
    # Relationships
    sdk_tokens:   Mapped[list["SDKToken"]]    = relationship(back_populates="developer", cascade="all, delete-orphan")
...
class SDKToken(Base):
    ...
    token_hash:   Mapped[str]       = mapped_column(String(64), unique=True, nullable=False)  # SHA-256 of actual token
...
class Wallet(Base):
    ...
    id:              Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    developer_id:    Mapped[uuid.UUID] = mapped_column(ForeignKey("developers.id"), unique=True, nullable=False)
...
class Transaction(Base):
    ...
    id:               Mapped[uuid.UUID]      = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    idempotency_key:  Mapped[str]            = mapped_column(String(36), unique=True, nullable=False)  # UUID from caller
    wallet_id:        Mapped[uuid.UUID]      = mapped_column(ForeignKey("wallets.id"), nullable=False)
    developer_id:     Mapped[uuid.UUID]      = mapped_column(ForeignKey("developers.id"), nullable=False)
    amount_credits:   Mapped[int]            = mapped_column(BigInteger, nullable=False)  # negative = debit
    type:             Mapped[TransactionType]= mapped_column(Enum(TransactionType), nullable=False)
    status:           Mapped[TransactionStatus] = mapped_column(Enum(TransactionStatus), nullable=False)
    api_call_id:      Mapped[uuid.UUID | None]  = mapped_column(UUID(as_uuid=True), ForeignKey("api_calls.id"), nullable=True)
...
class APIProvider(Base):
    ...
    id:              Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    slug:            Mapped[str]       = mapped_column(String(100), unique=True, nullable=False)  # e.g. "serpapi"
    name:            Mapped[str]       = mapped_column(String(255), nullable=False)
    description:     Mapped[str]       = mapped_column(Text, nullable=False)
    category:        Mapped[str]       = mapped_column(String(100), nullable=False)  # search | scraping | dataset | ai-model
    ...
    auth_type:       Mapped[str]       = mapped_column(String(50), default="api_key")  # api_key | bearer | basic
...
class APICall(Base):
    ...
    id:            Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    developer_id:  Mapped[uuid.UUID] = mapped_column(ForeignKey("developers.id"), nullable=False)
    provider_id:   Mapped[uuid.UUID] = mapped_column(ForeignKey("api_providers.id"), nullable=False)
    sdk_token_id:  Mapped[uuid.UUID] = mapped_column(ForeignKey("sdk_tokens.id"), nullable=False)
